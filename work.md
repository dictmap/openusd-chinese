...
## 第 445 轮：PromotionRound

- 轮次性质：页面晋级，exactly 1 个目标页。
- 轮次目的：记录本轮真实晋级结果，并保持报告、入口、manifest 与验证链一致。
- 本轮目标：`full_site/api/_usd_skel__intro.html`
- 官方页面：`https://openusd.org/release/api/_usd_skel__intro.html`
- source parity：`reports/round_445_usd_skel_intro_source_parity.json`
- commit SHA：`本轮提交后以最终回执为准`
- 完成数状态：good_bilingual=224；review_ready_zh=161。
...
- API complete / review_ready_zh：98 / 38
...
建议目标：`下一轮建议 PromotionRound：重新读取 inventory 后选择一个仍为 bilingual_draft 且有 source snapshot 的高价值 API 页面。`。如果该页无法达到 `good_bilingual`，停止并报告阻塞；不要重复处理 `full_site/api/_usd_skel__intro.html` 或 release 已完成页。
