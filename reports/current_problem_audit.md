...
Generated: 2026-06-07T22:59:47.161Z
...
| --- | --- | --- | --- |
| `P0-completion-stalled` | P0 | The promotion path now raises completion to 100/406, but the remaining 306 draft pages still need real upgrades. | Continue using the promotion manifest only for pages with Chinese main-reading-path coverage, paragraph-level structure, and draft-marker removal; do not mark guide-only drafts as complete. |
...
| `P0-automation-wrong-objective` | P0 | The automation must not regress to count-neutral refinement rounds. | Keep the automation prompt aligned with current counts and the promotion mechanism; do not let it regress to count-neutral round churn. |
| `P1-left-navigation-reading-flow` | P1 | Fixed in round 396 and preserved in round 399: completed full_site pages expose a real local reading path with side navigation, breadcrumb, local release/API entrances, related pages, previous/next links, and explicit official-source jumps. | Keep inject_openusd_reading_flow_navigation.mjs and audit_openusd_reading_flow_navigation.mjs in the validation chain whenever promoted pages or navigation status changes. |
...
| `P1-draft-content-thin` | P1 | Most remaining draft pages contain Chinese guidance and term notes, not paragraph-level bilingual translation. 295 pages remain draft_needs_translation and 11 remain draft_template_only. | For selected high-value pages, replace or supplement guide-only blocks with dense Chinese main-reading-path coverage and then promote status only if the page passes quality and English debt audits. |
...
PromotionRound target full_site/release/user_guides/schemas/usdLux/CylinderLight.html, if it can become good_bilingual; otherwise stop and report the concrete blocker.
