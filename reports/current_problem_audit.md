...
完成数曾长期停在 8 页。现在 promotion path 已经把真实完成数提升到 50 页，但 356 页仍是草稿，不能描述成完整翻译。

证据：`reports/bilingual_completion_promotions.json` 记录 42 个晋级页；`reports/translation_quality_review.json` 报告 `good_bilingual=50`。
...
证据：`openusd_bilingual_final.html` 当前显示 50 complete / 356 incomplete drafts。
...
证据：345 页仍是 `draft_needs_translation`，11 页仍是 `draft_template_only`。
...
继续 PromotionRound，每轮只晋级 1 个高价值页面。建议下一页：`full_site/api/class_sdf_usdz_file_format.html`。
