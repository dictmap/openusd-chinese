...
完成数曾长期停在 8 页。当前 promotion path 已经把真实完成数提升到 52 页，但 354 页仍是草稿，不能描述成完整翻译。

证据：`reports/bilingual_completion_promotions.json` 记录 44 个晋级页面；`reports/translation_quality_review.json` 报告 `good_bilingual=52`。
...
证据：`openusd_bilingual_final.html` 当前显示 52 complete / 354 incomplete drafts。
...
证据：343 页仍是 `draft_needs_translation`，11 页仍是 `draft_template_only`。
...
继续 PromotionRound，每轮只晋级 1 个高价值页面。建议下一页：`full_site/api/class_usd_imaging_adapter_registry.html`。
