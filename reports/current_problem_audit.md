...
完成数曾长期停在 8 页。当前 promotion path 已经把真实完成数提升到 71 页，但剩余 335 页仍是草稿，不能描述成完整翻译。

证据：`reports/bilingual_completion_promotions.json` 记录 63 个晋级页面；`reports/translation_quality_review.json` 报告 `good_bilingual=71`。
...
证据：`openusd_bilingual_final.html` 当前显示 71 complete / 335 incomplete drafts。
...
证据：324 页仍是 `draft_needs_translation`，11 页仍是 `draft_template_only`。
...
继续 PromotionRound，建议目标：`full_site/api/tf_page_front.html`。如果它不能达到 `good_bilingual`，自动化必须停止并报告阻塞。
